...
export function HostsPage() {
  ...
  return <>
    ...
    {modalOpen && <Modal title={editing ? `编辑节点 · ${editing.name}` : '新增 SSH 节点'} onClose={() => setModalOpen(false)}><form className="form-grid" onSubmit={save}>
      ...
      <label>主机地址<input value={form.hostname} onChange={(event) => setForm({ ...form, hostname: event.target.value })} required /></label>
      <label>SSH 端口<input type="number" min="1" max="65535" value={form.port} onChange={(event) => setForm({ ...form, port: event.target.value })} required /></label>
      <label>SSH 用户<input value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} required /></label>
      <label>认证方式<select value={form.auth_type} onChange={(event) => setForm({ ...form, auth_type: event.target.value as HostForm['auth_type'] })}><option value="password">密码</option><option value="key">私钥路径</option></select></label>
